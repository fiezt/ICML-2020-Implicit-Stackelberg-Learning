...
results_dir = os.path.join(os.getcwd(), 'MoGResults')
...
num_iter = 60000
batch_size = 256
view_size = kl_size = eig_size = 8192
freq_log = 250
...
for step in range(1, num_iter+1):
    ...
    if step % freq_eigs == 0:
        G_loss, D_loss = gan_loss_mog(G=G,D=D, batch_size=eig_size, n_latent=n_latent, 
                                     real_builder=real_builder, objective=objective, device=device)
        A_eigs, D_eigs, D_reg_eigs, J_eigs, SC_reg_eigs, SC2_reg_eigs = calc_game_eigs([G_loss, D_loss], [G,D], regularization, k=6, precise=precise)
        log_eigs(A_eigs, D_eigs, D_reg_eigs, J_eigs, SC_reg_eigs, SC2_reg_eigs)
        A_eig_data.append(A_eigs); 
        D_eig_data.append(D_eigs);
        D_reg_eig_data.append(D_reg_eigs);
        J_eig_data.append(J_eigs);
        SC_reg_eig_data.append(SC_reg_eigs)
        SC2_reg_eig_data.append(SC2_reg_eigs)
        store_eigs(A_eig_data, D_eig_data, D_reg_eig_data, J_eig_data, SC_reg_eig_data, SC2_reg_eig_data, info_dir)